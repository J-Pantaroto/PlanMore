...
const DropdownLink = ({ className = '', children, to, href, ...props }) => {
  const base =
    'block w-full px-4 py-2 text-start text-sm leading-5 text-gray-700 transition duration-150 ease-in-out hover:bg-gray-100 focus:bg-gray-100 focus:outline-none ';

  if (to) {
    return (
      <RouterLink to={to} {...props} className={base + className}>
        {children}
      </RouterLink>
    );
  }
  ...
  return (
    <a href={href} {...props} className={base + className}>
      {children}
    </a>
  );
};
...
const DropdownButton = ({ className = '', children, ...props }) => {
  ...
  return (
    <button type="button" {...props} className={base + className}>
      {children}
    </button>
  );
};
